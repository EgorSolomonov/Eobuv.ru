...
(function() {
	...
	BX.Sale.OrderAjaxComponent = {
		...
		bindEvents: function()
		{
			//BX.bind(this.orderSaveBlockNode.querySelector('order__submit'), 'click', BX.proxy(this.clickOrderSaveAction, this));
		...
		editPickUpList: function(isNew)
		{
			...
			if (!recommendList || !recommendList.querySelector('.bx-soa-pickup-list-item.bx-selected'))
			{
				...
				found = true;
				// Для функционала фильтрации складов по городам с условием, что если есть совпадение выводим один шаблон, иначе другой.
				let city_id = document.querySelector('.js-region-input input[name="ORDER_PROP_6"]').value;
				// Переменная для цикла.
				let sovpad = 0
				// Проходимся циклом по всем складам.
				for(i = 0; i < this.pickUpPagination.currentPage.length; i++){
					currentStore = this.pickUpPagination.currentPage[i];
					if(city_id === currentStore.DESCRIPTION){
						// Если в input значение равно нашему пользовательскому коду в админке, то делаем +1
						sovpad++;
					}
				}
				// Если совпадений больше 0 (хотя бы одно), то выводим шаблон из функции createPickUpItem_sovpad.
				if(sovpad > 0){
					for (i = 0; i < this.pickUpPagination.currentPage.length; i++)
					{
						currentStore = this.pickUpPagination.currentPage[i];
						...
						storeNode = this.createPickUpItem_sovpad(currentStore, {selected: currentStore.ID == selectedStore});
						pickUpList.appendChild(storeNode);
					}
				}	
				// Иначе выводим стандартный - createPickUpItem.
				else{
					for (i = 0; i < this.pickUpPagination.currentPage.length; i++)
					{
						currentStore = this.pickUpPagination.currentPage[i];
		...
		{
			$('#bx-soa-pickup .form-error').remove();
			if(document.querySelector('.bx-soa-pickup-list-item.hide.bx-selected')){
				$('html, body').animate({
					scrollTop: $("#bx-soa-pickup").offset().top - 100
					}, 1000); 
			$('#bx-soa-pickup').append('<div class="form-error" style="display: block;">Поле обязательно для заполнения</div>')
				return false;
			}
