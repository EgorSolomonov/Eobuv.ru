...
(function() {
	...
	BX.Sale.OrderAjaxComponent = {
		...
		editActiveDeliveryBlock: function(activeNodeMode)
		{
			...
			{
				deliveryContent = BX('js-delivery-errors');
				if (!deliveryContent)
				{
					deliveryContent = BX('js-delivery-errors');
		...
		editDeliveryItems: function()
		{
			...
			$(this.deliveryPagination.currentPage).each(function () {
				...
				let deliveryItem =
					...
					'    <input type="radio" name="DELIVERY_ID" value="'+this.ID+'" id="delivery-type-'+this.ID+'"'+checked+'>\n' +
					'    <label for="delivery-type-'+this.ID+'">\n' +
					'        <span class="delivery-type__name">'+deliveryName+'</span>\n' +
					'        <span class="delivery-type__desc">('+this.OWN_NAME+')</span>\n' +
					'    </label>\n' +
					'</div>';
				deliveryItemsContainer.append(deliveryItem);
				BX.bind(BX('delivery-type-'+this.ID), 'click', BX.proxy(that.selectDelivery, that));
			});
		},
		...
		editFadeDeliveryBlock: function()
		{
			...
			this.editFadeDeliveryContent(BX('js-delivery-errors'));
		},
